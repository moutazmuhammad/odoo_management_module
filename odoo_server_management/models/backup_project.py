...
class BackupProject(models.Model):
    ...
    def action_test_storage(self):
        """Verify credentials, then bucket access. Enumerates the Spaces the key
        can actually see so a 404 (DO returns it when the key can't access the
        Space) is diagnosed precisely."""
        self.env['server.stage']._check_access(GROUP_ADMIN)
        self.ensure_one()
        bucket = (self.bucket or '').strip()
        cli = self._boto_client()

        # 1. Authenticate + list the Spaces this key can access (account-wide).
        try:
            accessible = [b['Name'] for b in cli.list_buckets().get('Buckets', [])]
        except Exception as exc:  # noqa: BLE001 — almost always a credential error
            code = ((getattr(exc, 'response', None) or {}).get('Error') or {}).get('Code')
            raise UserError(_(
                "❌ Credentials rejected [%s]: %s\nThe access/secret key is wrong "
                "(use a DigitalOcean **Spaces key**, not an API token).")
                % (code or 'auth', exc))

        # 2. Is the target Space among them?
        if bucket not in accessible:
            seen = ', '.join(sorted(accessible)) or '(none)'
            raise UserError(_(
                "❌ The key authenticated, but cannot see a Space named '%s'.\n"
                "Spaces this key CAN access: %s\n"
                "→ Check the exact Space name, and that the key was created in the "
                "SAME DigitalOcean account/team as the Space.") % (bucket, seen))

        # 3. Name is accessible — confirm the region by listing it.
        try:
            ...
            if region_hdr and region_hdr != (self.region or '').strip():
                raise UserError(_(
                    "❌ Space '%s' exists but is in region '%s', not '%s'. "
                    "Set Region to '%s'.") % (bucket, region_hdr, self.region, region_hdr))
            raise UserError(_("❌ Space '%s' found but not readable: %s") % (bucket, exc))

        return self.env['server.stage']._notify(
            _("✅ Connected to Space '%s' (%s).") % (bucket, self.region))
